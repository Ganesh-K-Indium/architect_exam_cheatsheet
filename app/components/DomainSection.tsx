...
function ExamTip({ tip, colorKey }: { tip: string; colorKey: string }) {
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-lg ${colors.bg} border ${colors.border} border-opacity-40`}>
      <div className={`flex-shrink-0 w-5 h-5 rounded flex items-center justify-center text-xs font-bold ${colors.badge}`}>
        !
      </div>
      <p className={`text-xs leading-relaxed ${colors.text} font-medium`}>
        <ConceptText text={tip} />
      </p>
    </div>
  );
}
...
function ConceptList({ items, colorKey }: { items: string[]; colorKey: string }) {
  const colors = domainColors[colorKey as keyof typeof domainColors];
  return (
...
function Comparison({ comparison }: { comparison: NonNullable<Subsection["comparison"]> }) {
  return (
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        <div className="rounded-lg border border-red-500/30 bg-slate-950 overflow-hidden">
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 border-b border-red-500/20">
            ...
            <span className="text-xs text-red-400 font-semibold">Wrong</span>
          </div>
...
function SubsectionCard({ subsection, colorKey }: { subsection: Subsection; colorKey: string }) {
  const colors = domainColors[colorKey as keyof typeof domainColors];
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
      ...
      <div className={`flex items-center gap-3 px-4 py-3 border-b border-slate-800 ${colors.bg}`}>
        ...
        <h3 className={`text-sm font-semibold ${colors.text}`}>{subsection.title}</h3>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {/* Comparison */}
        {subsection.comparison && <Comparison comparison={subsection.comparison} />}
...
export default function DomainSection({ domain }: { domain: Domain }) {
  ...
  return (
    <section id={domain.id} className="scroll-mt-16 px-4 max-w-7xl mx-auto mb-16">
      {/* Domain header */}
      <div className={`relative rounded-2xl border-2 ${colors.border} overflow-hidden mb-6`}>
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-6">
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl ${colors.badge} flex items-center justify-center text-xl font-black shadow-lg`}>
              {domain.number}
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${colors.tag} ${colors.tagText}`}>
              ...
              <h2 className={`text-xl sm:text-2xl font-bold ${colors.text}`}>{domain.title}</h2>
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {domain.subsections.map((subsection) => (
          <SubsectionCard key={subsection.code} subsection={subsection} colorKey={domain.colorKey} />
        ))}
      </div>
    </section>
  );
